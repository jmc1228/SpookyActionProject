...
//serial communication between each ball's Arduino 
const int receive = 0; 
...
//this is the state the balls collapse to if the user is looking. It is randomized each time
int collapsed_state = 0; 
int collapsed_state_of_other_ball = 255; 
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(breakBeamPin), breakBeamCheck, CHANGE);
...
void lookingCheck(){
  ...
  if(lookingtemp != looking){
    ...
    //if there is a candidate change already, and it is longer than the threshold
    if(((now - candidate_change) > CANDIDATETHRESHOLD)){
        //there really was a looking change!
        ...
        if(lookingtemp == true){
            ...
            Serial.write(collapsed_state_for_other_ball); 
        }
